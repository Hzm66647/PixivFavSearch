...
INDEX = r"""<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>PixivFavSearch — 整合版</title><style>
...
<script>
const W=[{id:1,title:'桜色の季節',author:'あおい',h:280,color:'#e8b4bc',safe:true},{id:2,title:'Cyber Night',author:'neo_dev',h:200,color:'#4a90d9',safe:true},{id:3,title:'星空の下で',author:'ほしぞら',h:320,color:'#2c3e6e',safe:true},{id:4,title:'Neon City',author:'cyber_punk',h:240,color:'#9b59b6',safe:false},{id:5,title:'春の訪れ',author:'haru',h:260,color:'#f39c12',safe:true},{id:6,title:'Ocean Blue',author:'aqua',h:300,color:'#1abc9c',safe:true},{id:7,title:'落日',author:'sunset',h:220,color:'#e74c3c',safe:false},{id:8,title:'Silent Forest',author:'forest',h:280,color:'#27ae60',safe:true},{id:9,title:'未来都市',author:'future',h:250,color:'#3498db',safe:true},{id:10,title:'夢見る猫',author:'neko',h:310,color:'#e91e63',safe:true},{id:11,title:'Ancient Temple',author:'history',h:230,color:'#795548',safe:false},{id:12,title:'Rainy Day',author:'ame',h:270,color:'#607d8b',safe:true}];
const F=[{name:'✨ 精选',count:156,color:'#c77dff',works:[0,2,4,7,9]},{name:'🎮 游戏',count:42,color:'#4a90d9',works:[1,3,8]},{name:'🌸 二次元',count:387,color:'#e8b4bc',works:[0,4,6,9,11]},{name:'❤️ 喜欢',count:89,color:'#ff6b6b',works:[0,5,9]}];
let safe=true;
...
function renderWall(){document.getElementById('wall').innerHTML=wallHTML(safe?W.filter(w=>w.safe):W)}
function renderFavs(){document.getElementById('fav-grid').innerHTML=F.map((f,i)=>`<div class="fav-card" onclick="openFav(${i})"><div class="fav-card-bg" style="background:linear-gradient(135deg,${f.color},${f.color}88)"></div><div class="fav-card-info"><div class="fav-card-name">${f.name}</div><div class="fav-card-count">${f.count} 幅</div></div></div>`).join('')}
...
function openFav(idx){window.currentFavIdx=idx;go('inner')}
...
function doImport(){const b=document.getElementById('prog');b.classList.remove('active');b.style.width='0%';requestAnimationFrame(()=>{b.classList.add('active')});setTimeout(()=>{b.classList.remove('active');b.style.width='0%'},8000)}
