...
def _get_cached_search(q, tag, colt, mode):
    return _search_cache.get((q, tag, colt, mode))

def _set_cached_search(q, tag, colt, mode, result):
    key = (q, tag, colt, mode)
    ...
    while len(_search_cache_order) > _SEARCH_CACHE_MAX:
        del _search_cache[_search_cache_order.pop(0)]
...
class H(BaseHTTPRequestHandler):
    ...
    def _handle_get(self):
        ...
        if u.path == "/":
            ...
            _set_cached_search(q, tagf, colt, mode, merged)
...
<style>
 ...
 .pagination button{min-width:36px;height:36px;border:none;border-radius:8px;background:var(--card);color:var(--text);cursor:pointer;font-weight:600;box-shadow:var(--shadow);font-size:14px}
 .pagination button.active{background:var(--accent);color:var(--accent-ink)}
...
<div id=meta></div>
  <div id=pagination class=pagination></div><div id=grid class=grid></div>
...
async function go(){
 ...
 g.innerHTML=d.items.map(it=>{
  if(DATASRC==='demo'){
   ...
   <img loading=lazy decoding=async data-src="/thumb/${it.id}?lang=${LANG}" onerror="this.onerror=null;this.style.visibility='hidden'">
  ...
  return `<div class=card>
   <a href="https://www.pixiv.net/artworks/${it.id}">
     <img loading=lazy decoding=async data-src="/thumb/${it.id}" onerror="this.onerror=null;this.style.visibility='hidden'">
...
function updatePagination() {
  ...
  if (!pag || _totalPages <= 1) { if(pag) pag.style.display='none'; return; }
  pag.style.display = '';
  pag.innerHTML = '';
  const mk = (txt, fn, dis) => { const b = document.createElement('button'); b.textContent = txt; b.disabled = dis; b.onclick = fn; pag.appendChild(b); };
  mk('‹', () => { _currentPage--; goPage(_currentPage); }, _currentPage <= 0);
  for (let i = 0; i < _totalPages && i < 10; i++) {
    const b = document.createElement('button');
    b.textContent = i + 1;
    b.className = i === _currentPage ? 'active' : '';
    b.onclick = () => { _currentPage = i; goPage(i); };
    pag.appendChild(b);
  }
  mk('›', () => { _currentPage++; goPage(_currentPage); }, _currentPage >= _totalPages - 1);
}

async function goPage(page) { _currentPage = page; await go(); }
